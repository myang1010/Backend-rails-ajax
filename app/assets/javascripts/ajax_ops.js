function handle_ajax(event) {
    ...
    const factInput = document.getElementById('fact_text')
    ...
    const factInput1 = document.getElementById('fact_text1')
    ...
    restOpsDiv.addEventListener('click', (event) => {
      if (event.target === listUsersButton) {
      ...
      } else if (event.target === addFactButton) {
        var dataObject = {
          fact_text: factInput.value,
        ...
        var dataObject = {
          fact_text: factInput1.value,
          likes: likesInput1.value
        }
        if (dataObject.fact_text === "") {  // blank usernames not supported
          delete dataObject.fact_text;
        }
        if (dataObject.likes === "") { // blank passwords not supported
        ...
        if (userID5.value != "" && factID1.value != ""){
            ...
            alert(`Both user_id and fact_id are required.`);
          }    
      } 
    });
  }
